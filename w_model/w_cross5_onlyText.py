...
dataset = np.load(path.join(path.dirname(__file__), '..', 'w_np_data', 'vectors_label_textvec.npy'))
print(dataset.shape)
# print(dataset[:, 1])
...
for train_chunk_number in range(5):
    ...
    train, test = k_cross_w(dataset, train_chunk_number)
    ...
    def train_text():
        ...
        model.fit(X_train, dummy_Y_train, shuffle=True, epochs=15, batch_size=64,
        ...
        print(print_conf_matrix(conf_matrix(dummy_Y_test,predictions,label_index)))
