...
weibo_pos = pd.read_table(path.join(path.dirname(__file__),'..','data','weibo_pos_dup.txt'), header=None, sep='\n', encoding='utf8')
weibo_pos['label'] = 1
all_ = all_.append(weibo_pos, ignore_index=True)
weibo_neg = pd.read_table(path.join(path.dirname(__file__),'..','data','weibo_neg_dup.txt'), header=None, sep='\n', encoding='utf8')
weibo_neg['label'] = 0
...
pd.to_pickle(all_, path.join(path.dirname(__file__),'..','np_data','adddup_jd_words_doc2num_pd'))
pd.to_pickle(dict_index, path.join(path.dirname(__file__),'..','np_data','adddup_jd_word2seq_pd'))
...
np.save(path.join(path.dirname(__file__),'..','np_data','adddup_jd_embedding_weights'),embedding_weights)
...
np.save(path.join(path.dirname(__file__),'..','np_data','adddup_jd_doc2num'), X)
np.save(path.join(path.dirname(__file__),'..','np_data','adddup_jd_label'), Y)
...
model.save_weights(path.join(path.dirname(__file__),'..','model','adddup_sentiment_lstm.h5'),overwrite=1)
...
pd.to_pickle(text, path.join(path.dirname(__file__),'..','np_data','adddup_label_text_sentistatics_pd'))
...
print(senti.shape)
np.save(path.join(path.dirname(__file__),'..','np_data','adddup_vectors_senti.npy'), senti)
print('end')
