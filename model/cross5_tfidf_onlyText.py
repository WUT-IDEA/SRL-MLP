...
tfidf = np.load(path.join(path.dirname(__file__), '..', 'np_data', 'tfidf_textvec.npy'))
dataset = np.concatenate((label, tfidf), axis=1)
...
for train_chunk_number in range(5):
    ...
    X_train = train[:, 1: (w2v_dim + 1)]
    X_test = test[:, 1: (w2v_dim + 1)]
    Y_train = train[:, 0]
    Y_test = test[:, 0]
    ...
    def train_text():
        ...
        x = Dense(200, input_dim=w2v_dim, activation='relu')(input)
        x = Dropout(0.4)(x)
        ...
        model.fit(X_train, dummy_Y_train, shuffle=True, epochs=20, batch_size=32,
