...
np.save(path.join(path.dirname(__file__),'..','np_data','smote_JDMB_jd_doc2matrix'), X)
np.save(path.join(path.dirname(__file__),'..','np_data','smote_JDMB_jd_label'), Y)
...
model.fit(X[:train_num], Y[:train_num], validation_split=0.2, batch_size = batch_size,
          epochs=36, callbacks=[early_stopping])


model.save_weights(path.join(path.dirname(__file__),'..','model','smote_JDMB_sentiment_lstm.h5'),overwrite=1)
...
pd.to_pickle(text, path.join(path.dirname(__file__),'..','np_data','smote_JDMB_label_text_sentistatics_pd'))
...
print(senti.shape)
np.save(path.join(path.dirname(__file__),'..','np_data','smote_JDMB_vectors_senti.npy'), senti)
print('end')
