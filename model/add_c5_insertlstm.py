...
# 准备微博的lstm的数据
dict_index = pd.read_pickle(path.join(path.dirname(__file__), '..', 'np_data', 'add_jd_word2seq_pd'))
n_symbols = len(dict_index) + 1
embedding_weights = np.load(path.join(path.dirname(__file__), '..', 'np_data', 'add_jd_embedding_weights.npy'))
...
for n_epochs in [20,30,40,50,60,70,75,80,90,100,150,200,250,300]:
    ...
    for train_chunk_number in range(5):
        ...
        def train_insertlstm():
            ...
            lstm_model.load_weights(path.join(path.dirname(__file__), 'add_sentiment_lstm.h5'), by_name=True)
