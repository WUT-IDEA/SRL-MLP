...
from code_pre.my_modules import auc, conf_matrix, k_cross, print_dic, print_conf_matrix, decom_pca
...
senti = np.load(path.join(path.dirname(__file__), '..', 'np_data', 'smote_onlyJD_vectors_senti.npy'))

decom_dense_dim = 20
decom_dense = decom_pca(senti[:,134:],decom_dense_dim)
print('decom_dense_dim = ',decom_dense_dim)

dataset = np.concatenate((dataset,senti,decom_dense), axis= 1)
print(dataset.shape)
...
f = open(path.join(path.dirname(__file__),'..','record','temp_onlyJD.txt'), 'w')
...
for n_epochs in [650,700,750,800]:
    ...
    # 仅加入dense层训练
        ...
        def train_text_decomdense_1():
            input_text = Input(shape=(100,))
            input_decomdense = Input(shape=(decom_dense_dim,))

            input_text_dense = keras.layers.concatenate([input_text, input_decomdense])
            text_out = Dense(70, input_dim=(100+decom_dense_dim), activation='relu')(input_text_dense)
            text_out = Dropout(0.4)(text_out)
            text_out = Dense(15, activation='relu')(text_out)
            y = Dense(label_categories, activation='sigmoid',name='text_out')(text_out)
            ...
            model = Model(inputs = [input_text, input_decomdense], outputs = y)
            model.compile(optimizer='adam',
                          loss='binary_crossentropy',
                          metrics=['accuracy', auc])

            model.fit([X_train_text, X_train_decomdense], dummy_Y_train,shuffle=True,
                      validation_split=0.2, epochs=n_epochs, batch_size=64)


            scores_train = model.evaluate([X_train_text, X_train_decomdense], dummy_Y_train, batch_size=64)
            print('train_result => ' + model.metrics_names[0] + ": " + str(scores_train[0]) + "  "
                  + model.metrics_names[1] + ": " + str(scores_train[1]) + "  "
                  + model.metrics_names[2] + ": " + str(scores_train[2]) + "  "
                  )
            scores = model.evaluate([X_test_text, X_test_decomdense], dummy_Y_test, batch_size=64)
            print('test_result => ' + model.metrics_names[0] + ": " + str(scores[0]) + "  "
                  + model.metrics_names[1] + ": " + str(scores[1]) + "  "
                  + model.metrics_names[2] + ": " + str(scores[2]) + "  "
                  )
            ...
            Y_predict = model.predict([X_test_text, X_test_decomdense])
            confusion_matrix = conf_matrix(dummy_Y_test, Y_predict, label_index)
            print(confusion_matrix)
            print('round ' + str(train_chunk_number) + ' finished')
            return score_dic, confusion_matrix
        ...
        elif command == 'decom_dense':
            score_list_1, confusion_matrix_1 = train_text_decomdense_1()
        elif command == 'senti_lstm':
            score_list_1, confusion_matrix_1 = train_text_senti_lstm()
        else:
            print(command, 'error')
            exit(1)
